...
const Gallery = () => {
  ...
  return (
    <div id='combos' className="">
      {/* Desktop Gallery Container */}
      
<div className='hidden md:block w-full py-8 px-4 md:px-6'>
  <div className="max-w-5xl mx-auto">
    {/* Header Section */}
    <div className="text-center mb-8">
      <h1 className="font-extrabold text-4xl md:text-5xl">
        Alaimagal Hotels Culinary Combos
      </h1>
      <p className="mt-3 text-base md:text-lg text-gray-600 font-light italic">
        Savor the flavors of our exquisite dishes, crafted with love and tradition
      </p>
      <div className="mt-4 w-20 h-1 bg-gradient-to-r from-amber-400 to-orange-500 mx-auto rounded-full"></div>
    </div>

    <div className="flex flex-col md:flex-row gap-6">
      {/* Dynamic Main Image or Placeholder Text */}
      <div className="w-full md:w-7/12 flex flex-col gap-6 justify-center items-center">
        <div className="relative w-full rounded-xl overflow-hidden shadow-xl group bg-white">
          {mainImage ? (
            <div className="relative">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end">
                <div className="p-4">
                  <p className="text-white text-lg font-bold mb-1">{mainImage.alt}</p>
                  <div className="w-10 h-0.5 bg-amber-400"></div>
                </div>
              </div>
              {/* Decorative corner accent */}
              <div className="absolute top-3 right-3 w-6 h-6 border-t-2 border-r-2 border-amber-400 opacity-70"></div>
            </div>
          ) : (
            <div className="h-64 flex flex-col justify-center items-center p-6 text-center bg-gradient-to-br from-amber-50 to-orange-50">
              <div className="mb-3">
                <div className="w-12 h-12 mx-auto mb-3 rounded-full bg-gradient-to-r from-green-800 to-amber-500 flex items-center justify-center">
              ...
              <p className="text-base text-gray-600 leading-relaxed">
                From flaky parottas to aromatic cup rice and delicate idiyappam, our dishes are crafted with authentic flavors. Click a dish to explore!
              </p>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end">
            <div className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <p className="text-white text-lg font-bold">Best Selling</p>
              </div>
              <div className="w-12 h-0.5 bg-yellow-400 mt-1"></div>
            </div>
          </div>
          {/* Crown icon for best selling */}
          <div className="absolute top-3 left-3 w-6 h-6 bg-yellow-400 rounded-full flex items-center justify-center opacity-90">
      ...
      <div className="w-full md:w-5/12 flex flex-col gap-4">
        {galleryItems.map((item) => (
          <div
            key={item.id}
            className={`relative w-full rounded-lg overflow-hidden cursor-pointer transform transition-all duration-300 hover:scale-105 ${
              selectedCardId === item.id
                ? 'shadow-xl ring-2 ring-amber-400 ring-opacity-50 scale-105'
          ...
          >
            ...
            {selectedCardId === item.id && (
              <div className="absolute top-2 right-2 w-5 h-5 bg-amber-400 rounded-full flex items-center justify-center shadow-md">
            ...
            {selectedCardId === item.id && (
              <div className="absolute inset-0 rounded-lg border-2 border-amber-300 animate-pulse"></div>
            )}
          </div>
        ))}
      </div>
